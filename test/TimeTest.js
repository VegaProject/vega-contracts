...
let Time = artifacts.require("Time");


const verbose = false;

contract("Time", (accounts) => {
    ...
    it("should pass a time proposal and change the number of blocks in a Vega Period", async () => {
        ...
        await vega.executeTime(time.address)
        let currentTime = await vega.time()
        currentTime.toNumber().should.be.equal(60)
    })
});
